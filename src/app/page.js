...
import HomepageButtonControls from '@/components/home-page-button-controls';
// Note userInfo and Profil info is diff.
// User info means whether the user is authenticated or not
// Profile info means after authwntication whether the user is onboarded as a recruiter or taker
async function Home() {
  const user = await currentUser();
  // console.log(user);
  
  const profileInfo = await fetchProfileAction(user?.id);  //means current user is not onbaorded as a recruiter or candidate
